export const uz = {
  home: "Asosiy",
  production: "Mahsulotlar",
  aboutCompany: "О компании",
  photoGalary: "Fotogalereya",
  contact: "Aloqa",

  header1: "Yong'oq, quritilgan meva va sabzavotlarning ulgurji savdosi",
  header2: "Boshqa mamlakatlar uchun buyurtma bo'yicha yetkazib berish",
  header3:
    "Agrofirma O‘zbekiston Respublikasida yetishtiriladigan ho‘l meva-sabzavot, poliz va uzum, quritilgan sabzavot hamda meva va dukkakli ekinlarni yetkazib beradi (2021-2022)",

  about1: "Biz haqimizda",
  about2:
    "Asosiy eksport mahsulot turli navli quritilgan uzum, turli navli quritilgan oʻrik, quritilgan mevalar aralashmasi, quritilgan olma, quritilgan qovun, loviya, mos (yasmiq) kabi dukkakli ekinlar va boshqalar.",
  about3:
    "Bugungi kunda qishloq xo‘jaligi korxonalarimizda 500 nafardan ortiq hududlarimiz aholisi ish bilan ta’minlangan. Yurtdoshlarimiz yetishtirgan mahsulotlarni yetishtirish va ishlab chiqarishdan tashqari, bozorlarga ham qulay narxlarda va sifatli sotmoqda.",
  about4:
    "Kompaniyamiz 2020 yilda quritilgan mevalar va ikkilamchi ishlab chiqarilgan mevalarni sotish bo'yicha 12 ming tonnadan ortiq eksportga erishdi.",
  about5:
    "O‘tgan 2021-yilda esa quritilgan mevalar va ikkilamchi mahsulotlarni sotish 13,5 ming tonnadan ortiq eksportga yetdi.",
  about6:
    "Eksport mamlakatlari: Qozogʻiston, Rossiya, Qirgʻiziston, Tojikiston, Ukraina, Polsha, Belarus, Isroil, Iroq, Xitoy, Turkiya, arab mamlakatlari, Yevropa davlatlari va boshqalar.",

  why1: "Nega biz?",
  why2: "TOVAR NARXI VA SIFATNING OPTIMAL NOMONASI",
  why3: "HAMMA MAHSULOTLAR SERTIFIKATLANGAN VA XALQARO STANDARTLARGA MUVOFIQ",
  why4: "KENG ASSORTIMENT",
  why5: "UZLUKSIZ TA'MINOT",
  why6: "DOIMIY MIJOZLAR UCHUN AJOYIB CHEGIRMALAR SISTEMASI",

  benefit1: "Salomatlik. Quritilgan mevalarning foydalari",
  benefit2:
    "Quritilgan mevalar haqiqiy qutqaruvchidir, ular deyarli har qanday taomni tayyorlashda ishlatilishi mumkin. Va agar siz ozgina tajriba o'tkazsangiz va quritilgan mevali idishni yong'oq bilan bezang, siz mazali, sog'lom va chiroyli bo'lasiz.",
  benefit3: "Quritilgan mevalar haqida nimani bilishimiz kerak",
  benefit4:
    "Xurmo energiya manbai sifatida barcha quritilgan mevalardan ustundir. Ular E va H dan tashqari barcha vitaminlarni o'z ichiga oladi, lekin ular ayniqsa B5 ga boy, bu samaradorlikni oshiradi va konsentratsiyani oshiradi. Bundan tashqari, bu quritilgan mevalar tarkibida aspiringa o'xshash moddalar mavjud. Qadimgi tabiblar ularni shamollash va bosh og'rig'ini davolash uchun ishlatishlari tasodif emas.",
  benefit5:
    "Papayya oqsil almashinuvini faollashtiradi, immunitetni mustahkamlaydi, jinsiy faollikni oshiradi.",
  benefit6:
    "Armut tanadan og'ir metallar va toksinlarni olib tashlashga yordam beradi. U juda ko'p erimaydigan tolalarni o'z ichiga oladi, bu ichak faoliyatini normallashtiradi.",
  benefit7:
    "Olxo'ri balast moddalariga, shuningdek, B vitaminlariga boy.U uglevod almashinuvini optimallashtiradi, tashvishlarni yo'q qiladi va tananing stressli vaziyatlarga chidamliligini oshiradi.",
  benefit8:
    "Anjir qalqonsimon bez faoliyatini normallantiradi, shuningdek, saraton hujayralari rivojlanishining oldini oladi, bu so'nggi ilmiy tadqiqotlar bilan tasdiqlangan. Ovqat hazm qilishni rag'batlantiradigan fermentlarni o'z ichiga oladi. Xalq tabobatida u ichak parazitlarini olib tashlash va bronxitni davolashda muvaffaqiyatli qo'llaniladi.",
  benefit9:
    "Quritilgan o'rik, o'rik, kaisa kaliy, karotin, fosfor, kaltsiy, temir va B5 vitaminining ajoyib manbalari bo'lib, ular kuch berishdan tashqari, tanadagi yog'larni yoqishga ham yordam beradi. Bundan tashqari, quritilgan o'rikni muntazam iste'mol qilish saraton rivojlanish ehtimolini sezilarli darajada kamaytirishi ilmiy jihatdan isbotlangan.",
  benefit10: "Yong'oqlardan birinchi yordam to'plami",
  benefit11:
    "Bodom: Shirin bodomlar yuqori xolesterin, gipertoniya, o'sma kasalliklari, ko'z kasalliklari, semirib ketish yaralari, o't yo'llari kasalliklari va yurak kuyishi uchun tavsiya etiladi.",
  benefit12:
    "Shakar bilan bodom yo'tal, astma, plevrit va hemoptizi uchun foydalidir; achchiq bodom ayollar kasalliklari, yuqori nafas yo'llari, buyraklar kasalliklarini davolashda ishlatiladi.",
  benefit13:
    "Fındık surunkali charchoq, diabet, gipertoniya, prostata kengayishi, varikoz tomirlari, tomirlarning yallig'lanishi (flebit) bilan yordam beradi. Yuqori kaloriya tarkibi, vitaminlar va mikroelementlarning keng doirasi tufayli u semirib ketgan bemorlarga ko'rsatiladi (katta hajmlar kerak emas, shuning uchun oshqozonni cho'zmaydi). Asal va quritilgan o'rik bilan birgalikda - yurak-qon tomir tizimi kasalliklarining oldini olish uchun ajoyib vosita. Mayiz bilan maydalangan findiq surunkali kamqonlikni davolash uchun ishlatiladi.",
  benefit14:
    "Kaju psoriaz, anemiya, depressiya, metabolik kasalliklar uchun tavsiya etiladi; immunitet tizimini mustahkamlaydi, yurakning normal faoliyatini ta'minlaydi; qon xolesterin darajasini pasaytirishga yordam beradi.",
  benefit15:
    "Pista tetiklantiruvchi, tonik va tiklovchi ta'sirga ega; yurak urish tezligini va yurak kasalliklariga moyillikni kamaytirish; miya, jigar faoliyatiga foydali ta'sir ko'rsatadi.",
  benefit16:
    "Ular katta jismoniy zo'riqish davrida va jiddiy kasalliklardan keyin yordam beradi. Arterial gipertenziya, surunkali anemiya, sil kasalligi, tromboflebit bilan og'rigan odamlar uchun foydalidir.",
  benefit17:
    "Yong'oq asab to'qimalari, yurak, jigar va boshqa organlarning faoliyatini yaxshilaydi; hujayralarning o'sishi va yangilanishiga yordam beradi, qondagi xolesterin darajasini pasaytiradi; diuretik sifatida ishlatiladi. Charchoq, uyqusizlik uchun foydali; xotira, e'tibor va eshitishni yaxshilaydi; potentsialni, libidoni oshiradi. Har kuni 30 gramm yeryong'oq iste'mol qilish yurak-qon tomir kasalliklari xavfini sezilarli darajada kamaytiradi.",

  footer1: "Sertifikat",
  footer2: "Bog'lanish uchun ma'lumot",
  footer3:
    "Toshkent shahri Sergeli tumani, Korasuv MFI, Yangi turmush ko'chasi 4-uy",

  production1: "Quritilgan mevalarning foydalari",
  production2: `Qish yaqin, tanamizni "oziqlantirish" haqida jiddiy o'ylash vaqti keldi. Albatta, multivitaminli kavanoz hech qachon og'rimaydi, ammo tabiiy dorilar ham bor, ular ham haqiqiy davo hisoblanadi. Bu quritilgan mevalar va yong'oqlar.`,
  production3:
    "Quritilgan mevalarda kaltsiy, temir, magniy, shuningdek, tola va pektin kabi qimmatbaho iz elementlari to'liq mavjud bo'lib, bu foydali moddalarning haqiqiy konsentratidir. Misol uchun, kuniga besh bo'lak o'rik, anjir, xurmo yoki quritilgan o'rik iste'mol qilsangiz, ichak bilan bog'liq muammolarni abadiy unutasiz.",
  production4: "Mahsulotlar",

  companies1:
    "Asosiy Agrofirma ООО «URGUT AGRO-VET SERVIS» (Suxofrukt uchun) 2015-y",
  companies2:
    "ООО «ZARBDOR UNIVERSAL LOGISTIKA»  2017-y (Gidroponik issiqxona fermasida pomidor, bodring va boshqa yangi meva va sabzavotlar yetishtiriladi)",
  companies3: "ООО «ZARBDOR BARAKA AGROEKSPORT»  2020-y (Suxofrukt uchun)",

  catalog: "Katalog",

  aboutCompany1: "Bizning ishlab chiqarish printsipimiz",
  aboutCompany2:
    "Bizning ishlab chiqarish tamoyili har doim va o'z vaqtida sifatli, sog'lom yangi va quritilgan mevalarni ishlab chiqarish va etkazib berishdir.",
  aboutCompany3: "Kompaniya haqida",

  aboutContent1: "Maqsad",
  aboutContent2:
    "Korxonalarimiz tashkil etilishidan maqsad qishloq xo‘jaligi mahsulotlari – meva-sabzavot yetishtirish, ushbu mahsulotlarni qayta ishlash, mahalliy bozorlarni foydali, sifatli qishloq xo‘jaligi mahsulotlari bilan to‘ldirish va xalqaro bozorlarga chiqish edi. Va biz maqsadimizga erisha oldik.",
  aboutContent3:
    "Faoliyati davomida kompaniyamiz mahsulotlarimizni Xitoy, Polsha, Ukraina, Qozog'iston, Birlashgan Arab Amirliklari, Belarus, Isroil, Iroq va boshqa mamlakatlarga eksport qilish imkoniyatiga ega bo'ldi.",
  aboutContent4:
    "Bugungi kunda qishloq xo‘jaligi korxonalarimizda 500 nafardan ortiq hududlarimiz aholisi ish bilan ta’minlangan. Yurtdoshlarimiz yetishtirgan mahsulotlarni yetishtirish va ishlab chiqarishdan tashqari bozorlarga ham qulay narxlarda va sifatli sotmoqda.",
  aboutContent5:
    "Kompaniyamiz 2020 yilda quritilgan mevalar va ikkilamchi ishlab chiqarilgan mevalarni sotish bo'yicha 12 ming tonnadan ortiq eksportga erishdi.",
  aboutContent6:
    "O‘tgan 2021-yilda esa quritilgan mevalar va ikkilamchi mahsulotlarni sotish 13,5 ming tonnadan ortiq eksportga yetdi.",
  aboutContent7:
    "Asosiy eksport mahsuloti turli navli quritilgan uzum, turli navli quritilgan oʻrik, quritilgan oʻrik, quritilgan mevalar aralashmasi, quritilgan olma, quritilgan qovun, loviya, mos (yasmiq) kabi dukkakli ekinlar va boshqalar.",
  aboutContent8: "Bosh direktor",
  aboutContent9: "Maxqulov Soli Egamberdievich",
  aboutContent10:
    "S.Maxqulov 1966-yil 5-mayda O‘zbekiston Respublikasi Samarqand viloyati Urgut tumanida tug‘ilgan. Maʼlumoti oliy, 1988-yilda Toshkent xalq xoʻjaligi institutining iqtisod-moliya fakultetini tamomlagan.",
  aboutContent11:
    "Maxqulov Soli Egamberdievich – korxonaning asosiy asoschilaridan biri sifatida hozirgi bozor iqtisodiyoti, raqobat kuchini, talab va taklifni chuqur tushunadi. Uning ko‘p yillik tajribasi va bilimi bilan jamiyatning har qanday vaqtda to‘g‘ri va ijobiy faoliyat yuritishini ta’minlash mumkin. S.Maxqulov ko‘p yillardan buyon tadbirkorlik bilan shug‘ullanadi, savdo aloqalarini yaxshi tushunadi, ko‘plab xorijiy davlatlar bilan ko‘plab tashqi savdo aloqalari mavjud.",
  aboutContent12: "Direktor",
  aboutContent13: "Egamberdiev Raximjon Soliyevich",
  aboutContent14:
    "R.Egamberdiyev 1990 yil 28 iyunda O‘zbekiston Respublikasi Samarqand viloyati Urgut tumanida tug‘ilgan. O‘rta maxsus ma’lumotli, “ZARBDOR UNIVERSAL LOGISTIKA” MChJ direktori. R.Egamberdiyev yosh bo‘lishiga qaramay, qishloq xo‘jaligining ko‘plab yetakchi mamlakatlari – Belgiya, Gollandiya, Belarus va boshqa mamlakatlarda bo‘lgan. Tabiatan u har qanday sohada mas'uliyatli, mehnatsevar, qat'iyatli, maqsadli. Jamoada munosib obro'ga ega, do'stona va ochiqko'ngil.",
  aboutContent15:
    "R.Egamberdiev kompaniyamizning tashqi savdo aloqalari bo'yicha savdo vakili.",

  sertificates: "Sertifikatlar",

  photoGalary2: "Ishlar galareyasi",
  photoGalary3: "Eksport mamlakatlari:",
  photoGalary4:
    "Qozogʻiston, Rossiya, Qirgʻiziston, Tojikiston, Ukraina, Polsha, Belarus, Isroil, Iroq, Xitoy, Turkiya, arab davlatlari, Yevropa davlatlari va boshqalar.",

  contact1: "Kontaktlar",
  contact2: "MUROJAATINGIZNI QOLDIRING",
  contact3: "Ismingiz*",
  contact4: "Telefon raqamingiz*",
  contact5: "Murojaatingizni mavzusi*",
  contact6: "Yuborish",

  phone: "Aloqa",

  seeCatalog1: "Qo'ngiroq qilish",
  seeCatalog2: "Telegramdan yozish",
};
